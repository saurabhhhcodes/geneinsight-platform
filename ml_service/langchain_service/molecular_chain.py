    ...
    def _initialize_chains(self):
        ...
        try:
            ...
            sequence_prompt = PromptTemplate(
                input_variables=["sequence_type", "basic_analysis"],
                template="""Analyze {sequence_type} sequence. Basic: {basic_analysis}
...
3. Significance:
            ...
            docking_prompt = PromptTemplate(
                input_variables=["binding_affinity"],
                template="""Docking analysis. Affinity: {binding_affinity}
...
3. Next steps:
        ...
        try:
            ...
            if self.llm and 'docking' in self.chains:
                try:
                    ...
                    # Get LLM enhancement
                    best_affinity = docking_results.get('results', [{}])[0].get('affinity', 0)
                    llm_result = self.chains['docking'].run(
                        binding_affinity=f"{best_affinity} kcal/mol"
                    )
    ...
    def get_chain_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'device': 'cuda' if TRANSFORMERS_AVAILABLE and torch.cuda.is_available() else 'cpu' if TRANSFORMERS_AVAILABLE else 'none',
            'model_info': {
